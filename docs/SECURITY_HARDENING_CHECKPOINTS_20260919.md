...
- [ ] P0-02 관리자 목록 시간대 `Asia/Seoul` 명시 및 상세 화면과 일치 — 코드/CI PASS, 실제 로그인 데이터 화면 검증 대기
- [ ] P0-03 관리자 비로그인·로그인·상세 화면 회귀 테스트 — 비로그인/로그인 렌더 감사 PASS, 실제 로그인 상세 검증 대기
- [x] P0-04 CI: dependency audit / lint / image audits / typecheck / build / rendered audits PASS — CI #354
- [x] P0-05 Vercel Preview 후보 배포 READY 확인 — `83f0476` / `dpl_JKbpQTBte1DRwCHbbkevG4Gm6Xsr` READY, error/warning/fatal 로그 0건
...
## P1 — 애플리케이션 보안 강화
- [x] P1-01 Equipment 로그인/접수 Rate Limit을 분산 환경에서도 지속되는 방식으로 전환 — Supabase RPC/테이블 적용, service_role 전용 권한 및 제한 초과 동작 검증
- [x] P1-02 수급자 연동 fetch에 8초 timeout 및 안전한 `NEEDS_REVIEW` 처리 — 코드/CI/Preview READY
- [x] P1-03 인정서 업로드 magic-byte 검사(JPEG/PNG/WEBP/PDF) — 코드/CI/Preview READY
- [x] P1-04 Supabase `consultations`의 `anon`,`authenticated` 테이블 권한 명시적 revoke 검증 — 실제 DB 권한 재조회 PASS
- [x] P1-05 관리자/접수 API `private, no-store` 및 비인증 401/리다이렉트 회귀 테스트 — rendered audit PASS
