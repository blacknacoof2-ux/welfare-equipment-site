...
## 현재 단계
- `welfare-equipment-site`의 LINK-04 프록시 API는 로컬 production build PASS.
- LINK-05~09 및 LINK-12 UI/클라이언트 검증 코드는 구현 완료.
- 다음 게이트는 두 로컬 서버에서 동일한 `BENEFICIARY_INTEGRATION_SECRET`을 설정하고 실제 서버간 자격조회 및 화면 동작을 확인하는 것.
- 실제 조회 PASS 후 LINK-10에서 최종 접수 API가 서버에서 자격·남은수량을 재검증하도록 마감한다.
