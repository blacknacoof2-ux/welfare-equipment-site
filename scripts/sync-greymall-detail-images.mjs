...
function extractCandidateUrls(html) {
  ...
  const patterns = [
    ...
    /href=["']([^"']*goods_view\.php\?goodsNo=\d+[^"']*)["']/gi,
  ];

  for (const pattern of patterns) {
    let match;
    while ((match = pattern.exec(html)) && found.length < MAX_CANDIDATES) {
      const raw = decodeHtml(match[1]);
      const absolute = new URL(raw, BASE_URL).toString();
      const goodsNoMatch = absolute.match(/goodsNo=(\d+)/);
      if (!goodsNoMatch) continue;
      const canonical = `${BASE_URL}/goods/goods_view.php?goodsNo=${goodsNoMatch[1]}`;
...
function isExactProductMatch(product, pageTitle) {
  const model = normalizeModel(product.model);
  const name = normalizeModel(product.name);
  const title = normalizeModel(pageTitle);
  if (!model || !title) return false;

  if (model.length >= 4) return title.includes(model);
  // Very short model strings (for example IU/SS) are too ambiguous by themselves.
  return title.includes(model) && Boolean(name) && title.includes(name);
}
...
function getUnavailableReason(html, pageTitle) {
  ...
  if (/sold[\s_-]*out/i.test(topHtml) || /class=["'][^"']*soldout/i.test(topHtml)) {
    return 'soldout';
  }
  return null;
}
...
function getDetailSegment(html) {
  const starts = [
    html.indexOf('상품상세정보'),
    html.indexOf('상품 상세 정보'),
    html.indexOf('goods_detail_cont'),
...
function extractDetailImages(html) {
  ...
  // Fallback: Godomall long-form detail images consistently live under /data/editor/goods/.
...
async function inspectProduct(product, pageCache) {
  const model = String(product.model ?? '').trim();
  if (!model) return { status: 'NO_MODEL' };

  let searchHtml;
  try {
    searchHtml = await fetchText(`${SEARCH_URL}${encodeURIComponent(model)}`);
  } catch (error) {
    return { status: 'SEARCH_ERROR', error: String(error) };
  }

  const candidateUrls = extractCandidateUrls(searchHtml);
  if (candidateUrls.length === 0) return { status: 'NO_GREYMALL_MATCH' };
  ...
  for (const url of candidateUrls) {
    ...
    if (!isExactProductMatch(product, pageTitle)) continue;
    exactMatches.push({ url, html, pageTitle });
  }

  if (exactMatches.length === 0) {
    return { status: 'MODEL_MISMATCH', candidates: candidateUrls };
  }

  if (exactMatches.length > 1) {
    const titles = new Set(exactMatches.map((item) => normalizeModel(item.pageTitle)));
    if (titles.size > 1) {
      return {
        status: 'AMBIGUOUS_MATCH',
        candidates: exactMatches.map((item) => ({ url: item.url, title: item.pageTitle })),
      };
    }
  }

  const match = exactMatches[0];
  const unavailableReason = getUnavailableReason(match.html, match.pageTitle);
  ...
  const urls = extractDetailImages(match.html);
  if (urls.length === 0) {
    return {
      status: 'NO_DETAIL_IMAGES',
      sourceUrl: match.url,
      pageTitle: match.pageTitle,
    };
  }
...
async function main() {
  ...
  for (let index = 0; index < uniqueProducts.length; index += 1) {
    ...
    const result = await inspectProduct(product, pageCache);
    entries.push({
      index: index + 1,
      slug: product.slug,
      name: product.name,
  ...
  const detailImageCount = entries.reduce(
    (sum, entry) => sum + (entry.result.status === 'COMPLETE' ? entry.result.urls.length : 0),
    0,
  );
  ...
  console.log(JSON.stringify({ checkedAt, totalUniqueModels: uniqueProducts.length, detailImageCount, counts }, null, 2));
}
